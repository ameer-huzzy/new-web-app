...
async def upload_limo_payments(file: UploadFile = File(...), db: Session = Depends(get_db)):
    try:
        ...
        for index, row in df.iterrows():
            ...
            existing_payment = crud.get_limo_payment(db, payment_id)
            if existing_payment:
                existing_payments += 1
                continue  # Skip existing payments
        ...
        if existing_payments > 0 and new_payments == 0:
            return {
                "message": "File already exists - all records are duplicates",
                "filename": file.filename,
                "existing_records": existing_payments,
                "new_records": new_payments
            }
        else:
            return {
